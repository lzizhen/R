stepwise <- function(formula, data, alpha.enter = 0.15, alpha.remove = 0.15, ...) {
  showModel <- function(title, y, model) {
    cat("\n", title, ":\n", sep = "")
    formula <- paste(names(y), paste(names(model), collapse = ' + '), sep = ' ~ ')
    cat(formula)
    clm <- lm(formula, data = cbind(y, model))
    cat('\n\nCoefficients:\n')
  ...
  if (maxv[, 'F'] >= qf(alpha.enter, 1, maxv[, 'df'], lower.tail = FALSE)) {
    ...
    print(enter)
    showModel("Best model", y, extras$model)
    return(invisible())
  }
  ...
  if (length(x) == 0) {
    ...
    clm <- showModel("Current model", y, extras$model)
  }
